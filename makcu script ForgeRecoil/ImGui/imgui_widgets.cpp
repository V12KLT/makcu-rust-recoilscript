...
bool ImGui::ButtonEx(const char *label, const ImVec2 &size_arg,
  ...
  ImVec2 size =
      CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 31.0f,
                   label_size.y + style.FramePadding.y * 2.0f);

  // Move button to the right by 10 pixels
  pos.x += 24.0f;
  ...
  // Adjust text clipping position to match button shift
  ImVec2 text_pos = bb.Min + style.FramePadding;
  text_pos.x += 0.0f; // Move text to the right by 10 pixels

  RenderTextClipped(text_pos, bb.Max - style.FramePadding, label, NULL,
...
void ImGui::Scrollbar(ImGuiAxis axis) // scrollbar color
{
  ImGuiContext &g = *GImGui;
  ImGuiWindow *window = g.CurrentWindow;
...
bool ImGui::ScrollbarEx(const ImRect &bb_frame, ImGuiID id, ImGuiAxis axis,
  ...
  // Adjust the height of the scrollbar to make it shorter
  float adjustment = 20.0f; // Adjust this value based on how much shorter you
                            // want the scrollbar to be
  if (axis == ImGuiAxis_Y) {
    bb.Min.y += adjustment; // Move the top of the scrollbar down
    bb.Max.y -= adjustment; // Move the bottom of the scrollbar up
  } else if (axis == ImGuiAxis_X) {
    bb.Min.x += adjustment; // Move the left of the scrollbar to the right
    bb.Max.x -= adjustment; // Move the right of the scrollbar to the left
  }

  bb.Expand(
      ImVec2(-ImClamp(IM_FLOOR((bb_frame_width - 1.0f) * 0.5f), 0.0f, 3.0f),
  ...
  // Draw the background and grab to the ForegroundDrawList to ensure it's on
  // top
  ImDrawList *draw_list =
      window->DrawList; // or use window->ForegroundDrawList if you need it
                        // specifically on top
  draw_list->AddRectFilled(bb_frame.Min, bb_frame.Max, bg_col,
  ...
  draw_list->AddRectFilled(grab_rect.Min, grab_rect.Max, grab_col,
...
void DrawFadingRect(ImDrawList *draw_list, const ImRect &check_bb, float pad,
                    const ImColor &color, float t) {
  const int steps = 100;                 // Number of steps for the gradient
  const float min_alpha = 0.2f * 255.0f; // 20% of 255

  for (int i = 0; i < steps; ++i) {
    float alpha = (1.0f - (i / (float)steps)) * (t * 255);
    alpha = ImMax(alpha, min_alpha); // Ensure alpha doesn't drop below 20%
    float y_start =
        check_bb.Min.y + pad + 1 +
        ((check_bb.Max.y - check_bb.Min.y - 2 * pad - 2) * (i / (float)steps));
    float y_end = check_bb.Min.y + pad + 1 +
                  ((check_bb.Max.y - check_bb.Min.y - 2 * pad - 2) *
                   ((i + 2) / (float)steps));
    draw_list->AddRectFilled(
        ImVec2(check_bb.Min.x + pad + 1, y_start),
        ImVec2(check_bb.Max.x - pad - 1, y_end),
        ImColor(color.Value.x, color.Value.y, color.Value.z, alpha / 255.0f),
        0);
  }
}
...
bool ImGui::tab(const char *label, bool selected) {
  ...
  const float rect_bottom_extension =
      10.0f; // Extend the rectangle downward by 10 pixels
  const ImRect rect(pos, ImVec2(pos.x + label_size.x + tab_padding_x * 2,
                                pos.y + label_size.y + tab_padding_y * 2));
  ImGui::ItemSize(ImVec4(rect.Min.x, rect.Min.y, rect.Max.x + 2.f, rect.Max.y),
  ...
  if (selected) {
    ...
    if (it_alpha->second > 1.0f)
      it_alpha->second = 1.0f;
  } else {
    it_alpha->second -= fade_speed * ImGui::GetIO().DeltaTime;
    if (it_alpha->second < 0.0f)
      it_alpha->second = 0.0f;
  }
  ...
  ImColor target_color =
      ImColor(130, 130, 135, 255); // Default color for non-selected tabs

  if (selected) {
    target_color = ImColor(200, 30, 30, 255); // Crimson red for selected tabs
  }

  // Interpolate color towards the target
  float speed = 2.5f * ImGui::GetIO().DeltaTime;
  current_color = ImColor(ImLerp(current_color.Value.x * 255.0f,
                                 target_color.Value.x * 255.0f, speed) /
                              255.0f,
                          ImLerp(current_color.Value.y * 255.0f,
                                 target_color.Value.y * 255.0f, speed) /
                              255.0f,
                          ImLerp(current_color.Value.z * 255.0f,
                                 target_color.Value.z * 255.0f, speed) /
                              255.0f,
                          ImLerp(current_color.Value.w * 255.0f,
                                 target_color.Value.w * 255.0f, speed) /
                              255.0f);

  // Calculate text position with padding
  ImVec2 text_pos =
      ImVec2(rect.Min.x + tab_padding_x, rect.Min.y + tab_padding_y);
  ...
  if (selected || current_alpha > 0) {
    ...
    ImVec2 rect_filled_max =
        ImVec2(rect.Max.x, rect.Max.y + rect_bottom_extension);
    window->DrawList->AddRectFilled(rect.Min, rect_filled_max,
                                    ImColor(6, 6, 8, current_alpha));

    if (selected || current_alpha > 0) {
      // Define colors with the applied fade alpha
      ImColor line_color = ImColor(
          35, 10, 10, current_alpha); // Left and right line color with red tint
      ImColor top_line_color =
          ImColor(200, 30, 30, current_alpha); // Crimson red top line

      // Draw the left line with rounding
      window->DrawList->AddLine(
          ImVec2(rect.Min.x, rect.Max.y),
          ImVec2(rect.Min.x, rect.Min.y + rounding_radius), line_color,
          1.0f); // Thinner line
      window->DrawList->AddBezierCurve(
          ImVec2(rect.Min.x, rect.Min.y + rounding_radius),
          ImVec2(rect.Min.x, rect.Min.y),
          ImVec2(rect.Min.x + rounding_radius, rect.Min.y),
          ImVec2(rect.Min.x + rounding_radius, rect.Min.y), line_color,
          1.0f); // Thinner curve

      // Draw the right line with rounding
      window->DrawList->AddLine(
          ImVec2(rect.Max.x, rect.Max.y),
          ImVec2(rect.Max.x, rect.Min.y + rounding_radius), line_color,
          1.0f); // Thinner line
      window->DrawList->AddBezierCurve(
          ImVec2(rect.Max.x, rect.Min.y + rounding_radius),
          ImVec2(rect.Max.x, rect.Min.y),
          ImVec2(rect.Max.x - rounding_radius, rect.Min.y),
          ImVec2(rect.Max.x - rounding_radius, rect.Min.y), line_color,
          1.0f); // Thinner curve

      // Draw the flat pink line on top
      window->DrawList->AddLine(
          ImVec2(rect.Min.x + rounding_radius, rect.Min.y),
          ImVec2(rect.Max.x - rounding_radius, rect.Min.y), top_line_color,
          1.0f); // Straight top line
      ...
      window->DrawList->AddBezierCurve(
          ...
          ImVec2(rect.Min.x, rect.Min.y),
          ImVec2(rect.Min.x + rounding_radius, rect.Min.y),
          ImVec2(rect.Min.x + rounding_radius, rect.Min.y), top_line_color,
          1.0f); // Left curve
...
bool ImGui::subtab(const char *label, bool selected) { // subtab color
  ...
  const float tab_padding_x = 3.0f;
  const float tab_padding_y = 2.0f;
  const float text_padding_left = 5.0f; // Padding from the left for the text
  const ImRect rect(pos, ImVec2(pos.x + label_size.x + tab_padding_x * 2,
                                pos.y + label_size.y + tab_padding_y * 2));
  ImGui::ItemSize(ImVec4(rect.Min.x, rect.Min.y, rect.Max.x + 2.f, rect.Max.y),
                  style.FramePadding.y);
  ...
  ImVec2 text_pos =
      ImVec2(rect.Min.x + text_padding_left, rect.Min.y + tab_padding_y);
  ...
  if (selected) {
    target_color =
        ImColor(200, 30, 30, 255); // Crimson red for selected subtabs
  }
  ...
  ImColor line_color = ImColor(
      200, 30, 30,
      static_cast<int>(current_line_alpha)); // Crimson red with animated alpha
  ...
  window->DrawList->AddLine(line_start, line_end, line_color,
                            1.0f); // Thinner line with thickness of 1.0f

  // Draw multiple white lines with decreasing alphas
  const int num_white_lines =
      75; // Increased number of lines for a smoother gradient
  for (int i = 0; i < num_white_lines; ++i) {
    ...
    window->DrawList->AddLine(ImVec2(rect.Min.x + 1 + i, rect.Min.y),
                              ImVec2(rect.Min.x + 1 + i, rect.Max.y),
                              white_line_color, 1.0f);
  }
  ...
  ImGui::SetCursorPosY(ImGui::GetCursorPosY() + rect.GetHeight() +
                       style.ItemSpacing.y);
...
bool ImGui::Checkbox(const char *label, bool *v) {
  ...
  window->DrawList->AddRectFilled(
      ImVec2(check_bb.Min.x + pad + 2.5, check_bb.Min.y + pad + 2.5),
      ImVec2(check_bb.Max.x - pad - 2.5, check_bb.Max.y - pad - 2.5),
      ImColor(18, 18, 20));
  window->DrawList->AddRect(
      ImVec2(check_bb.Min.x + pad + 1, check_bb.Min.y + pad + 1),
      ImVec2(check_bb.Max.x - pad - 1, check_bb.Max.y - pad - 1),
      ImColor(40, 10, 10, alpha), 0);
  if (check_t > 0.0f) {
    window->DrawList->AddRectFilled(
        ImVec2(check_bb.Min.x + pad + 2.5, check_bb.Min.y + pad + 2.5),
        ImVec2(check_bb.Max.x - pad - 2.5, check_bb.Max.y - pad - 2.5),
        ImColor(200, 30, 30, int(check_t * 255)), 0);
  }
  ...
  if (label_size.x > 0.0f)
    window->DrawList->AddText(
        check_bb.Min + ImVec2(pad, pad) + ImVec2(18, -1.001),
        ImColor(250, 250, 250, static_cast<int>(current_alpha)), label);
...
bool ImGui::RadioButton(const char *label, bool active, const bool theme) {
  ...
  const ImRect total_bb(
      ...
      pos + ImVec2(square_sz + (label_size.x > 0.0f
                                    ? style.ItemInnerSpacing.x + label_size.x
                                    : 0.0f),
                   label_size.y + style.FramePadding.y * 1.0f));
  ItemSize(total_bb, style.FramePadding.y);
  if (!ItemAdd(total_bb, id))
    return false;
  float t = active ? 1.0f : 0.0f;
  float ANIM_SPEED = 1.2f; // Bigger = Slower
  ...
  if (!theme) {
    ...
    auto lb = label == "##black";
    window->DrawList->AddCircleFilled(
        center - ImVec2(0, 3), radius,
        lb ? ImColor(3, 16, 29) : ImColor(244, 244, 244), 24);
    if (active) {
      window->DrawList->AddCircle(center, radius + 1,
                                  ImColor(77, 125, 253, int(t * 255)), 24);
    }
  }
...
void arcs(int x, int y, int r1, int r2, int s, int d, ImColor col) {
  for (int i = s; i < s + d; i++) {
    ...
    ImGui::GetWindowDrawList()->AddLine(
        ImVec2(x + cos(rad) * r1, y + sin(rad) * r1),
        ImVec2(x + cos(rad) * r2, y + sin(rad) * r2), col);
  }
}
bool ImGui::ThemeSwitch(const char *label, bool *v) {
  ...
  const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
  RenderNavHighlight(total_bb, id);
  // RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ?
  // ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered :
  ...
  arcs(check_bb.Min.x + pad + label_size.x - 1, check_bb.Min.y + pad * 3.6, 0,
       radius, 90 + add, 180, ImColor(themeup[0], themeup[1], themeup[2], 255));
  window->DrawList->AddCircle(
      check_bb.Min + ImVec2(pad + label_size.x, pad * 3.6), radius,
      ImColor(themeup[0], themeup[1], themeup[2]), 32, 2);
  ...
  if (g.LogEnabled)
    LogRenderedText(&label_pos, mixed_value ? "[~]" : *v ? "[x]" : "[ ]");
  if (label_size.x > 0.0f)
    window->DrawList->AddText(label_pos - ImVec2(30, 0),
                              ImColor(220, 220, 220, 220), label);
...
void ImGui::Spacing() // combo spacing
{
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return;
...
bool ImGui::BeginCombo(const char *label, const char *preview_value,
                       ImGuiComboFlags flags, bool text) {
  ...
  const ImRect frame_bb(window->DC.CursorPos + ImVec2(30, text ? 10 : 0),
                        window->DC.CursorPos + ImVec2(-25.7, text ? 10 : 0) +
                            ImVec2(window->Size.x - 8,
                                   label_size.y + style.FramePadding.y * 2.0f));
  ...
  if (!(flags & ImGuiComboFlags_NoPreview)) {
    ...
    for (int i = 0; i < steps; ++i) {
      ...
      ImColor color_start =
          ImColor(ImLerp(col_top.Value.x, col_bottom.Value.x, t1),
      ...
      ImColor color_end =
          ImColor(ImLerp(col_top.Value.x, col_bottom.Value.x, t2),
      ...
      float y1 = ImLerp(frame_bb.Min.y, frame_bb.Max.y, t1);
      float y2 = ImLerp(frame_bb.Min.y, frame_bb.Max.y, t2);

      window->DrawList->AddRectFilledMultiColor(
          ImVec2(frame_bb.Min.x, y1), ImVec2(frame_bb.Max.x, y2), color_start,
          color_start, color_end, color_end);
    }

    window->DrawList->AddRect(
        frame_bb.Min, frame_bb.Max, ImColor(0, 0, 0, 100), 2,
        popup_open ? ImDrawFlags_::ImDrawFlags_RoundCornersTop
  ...
  if (!(flags & ImGuiComboFlags_NoArrowButton)) {
    ...
    //  frame_bb.Max, ImColor(33, 33, 33), 2, (w <= arrow_size) ?
    //  ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
    if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
      RenderArrow(window->DrawList,
                  ImVec2(value_x2 + style.FramePadding.y,
                         frame_bb.Min.y + style.FramePadding.y + 3),
                  text_col, popup_open ? ImGuiDir_Down : ImGuiDir_Up, 0.6f);
  }
  ...
  //  if (label_size.x > 0 && text)
  //  window->DrawList->AddText(ImVec2(window->DC.CursorPos.x + 8,
  //  window->DC.CursorPos.y - 28), ImColor(250, 250, 250, 200), label);
  Spacing();
  ...
  if (has_window_size_constraint) {
    ...
    SetNextWindowSizeConstraints(
        ImVec2(window->Size.x - 65, 0.0f),
        ImVec2(window->Size.x - 3,
...
bool ImGui::DragScalar(const char *label, ImGuiDataType data_type, void *p_data,
  ...
  if (!temp_input_is_active) {
    ...
    // FIXME: Currently polling ImGuiConfigFlags_IsTouchScreen, may either poll
    // an hypothetical ImGuiBackendFlags_HasKeyboard and/or an explicit drag
    // settings.
...
bool ImGui::SliderScalar(const char *label, ImGuiDataType data_type,
  ...
  const ImVec2 label_size = CalcTextSize(label, NULL, true);
  const ImRect frame_bb(
      window->DC.CursorPos + ImVec2(32.5, 10.6),
      window->DC.CursorPos +
          ImVec2(window->Size.x - 35,
                 20 + label_size.y + style.FramePadding.y * 1.3f));
  const ImRect total_bb(frame_bb.Min,
                        frame_bb.Max +
                            ImVec2(label_size.x > 0.0f
                                       ? style.ItemInnerSpacing.x + label_size.x
  ...
  ImVec2 outline_min = ImVec2(frame_bb.Min.x - outline_thickness,
                              frame_bb.Min.y + 8 - outline_thickness);
  ImVec2 outline_max = ImVec2(frame_bb.Max.x + outline_thickness,
                              frame_bb.Max.y - 5 - 9 + outline_thickness);
  window->DrawList->AddRect(outline_min, outline_max, ImColor(0, 0, 0, 85),
                            g.Style.FrameRounding, ImDrawFlags_RoundCornersAll,
                            outline_thickness);

  // Draw frame (main)
  RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 8),
              ImVec2(frame_bb.Max.x, frame_bb.Max.y - 5 - 9),
              ImColor(30, 30, 30), true, g.Style.FrameRounding);
  ...
  if (grab_bb.Max.x > grab_bb.Min.x) {
    window->DrawList->AddRectFilled(
        ImVec2(frame_bb.Min.x, frame_bb.Min.y + 8),
        ImVec2(grab_bb.Max.x, grab_bb.Max.y - 5 - 7), ImColor(112, 20, 0, 255),
        style.GrabRounding);
  }
  ...
  if (g.LogEnabled)
    LogSetNextTextDecoration("{", "}");
  window->DrawList->AddText(
      ImVec2(frame_bb.Max.x - 0 - CalcTextSize(value_buf).x,
             window->DC.CursorPos.y - 29),
      ImColor(255, 255, 255, static_cast<int>(current_alpha)), value_buf,
      value_buf_end); // Adjusted Y-coordinate

  if (label_size.x > 0.0f)
    window->DrawList->AddText(
        ImVec2(window->DC.CursorPos.x + 32, window->DC.CursorPos.y - 29),
        ImColor(255, 255, 255, static_cast<int>(current_alpha)), label);
...
static bool InputTextFilterCharacter(unsigned int *p_char,
  ...
  if (flags &
    ...
    // standard mandate that programs starts in the "C" locale where the decimal
    // point is '.'. We don't really intend to provide widespread support for
    // it, but out of empathy for people stuck with using odd API, we support
    // the bare minimum aka overriding the decimal point. Change the default
    // decimal_point with:
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf,
  ...
  const bool user_nav_input_start =
      (g.ActiveId != id) &&
      ((g.NavInputId == id) || (g.NavActivateId == id &&
  ...
  if (g.ActiveId == id) {
    ...
    if (apply_edit_back_to_user_buffer) {
      ...
      if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
        ...
        if ((flags & ImGuiInputTextFlags_CallbackHistory) != 0 &&
...
bool ImGui::ColorEdit4(const char *label, float col[4],
  ...
  IM_ASSERT(ImIsPowerOfTwo(
      flags & ImGuiColorEditFlags__InputMask)); // Check that only 1 is selected
...
bool ImGui::Selectable(const char *label, bool selected,
  ...
  //   hovered = true;
  // ImColor col = ((selected && hovered) ? ImColor(250, 192, 192) : hovered ?
  // ImColor(112, 146, 190, 100) : ImColor(112, 146, 190));
  //    if (hovered || selected)
  //    {
  ...
  ImColor t_col =
      ((selected && hovered) ? ImColor(112, 20, 0)
       : hovered  ? ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, 0.5f)
       : selected ? ImColor(112, 20, 0)
                  : ImColor(255, 255, 255, 100));

  // Popup text, popup color
  window->DrawList->AddText(text_min + ImVec2(4, 0), ImColor(t_col), label);
...
bool ImGui::TabItemEx(ImGuiTabBar *tab_bar, const char *label, bool *p_open,
  ...
  // **New Code for Clipping: Check if the tab is within the visible area before
  // rendering**
  const bool is_tab_visible = bb.Max.x >= tab_bar->ScrollingRectMinX &&
  ...
  if (want_clip_rect)
    PushClipRect(
        ImVec2(ImMax(bb.Min.x, tab_bar->ScrollingRectMinX), bb.Min.y - 1000),
  ...
  // menu automatically highlights the current widget.
